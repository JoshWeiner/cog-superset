...
:::warning Docker image support
The `superset mcp` CLI is **not** included in the publicly published `apache/superset:latest` (or other tagged) Docker images on Docker Hub. Running `superset mcp run` against those images will fail with `Error: No such command 'mcp'`.

To use MCP with Docker, build the image locally from this repository — `docker-compose.yml` and `docker-compose-light.yml` use a `build:` context that includes the MCP CLI and the `fastmcp` extra. See the [Docker Compose](#docker-compose) section below for an example.
:::
...
:::warning
The published `apache/superset:latest` image on Docker Hub does **not** ship the `superset mcp` CLI. Build the image from this repository (or another build that bundles the `fastmcp` extra and the `superset/cli/mcp.py` command) — otherwise `superset mcp run` will fail with `Error: No such command 'mcp'`.
:::
